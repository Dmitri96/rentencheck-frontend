...
export function PensionResultsOverview({ data, desiredPension }: PensionResultsOverviewProps) {
    ...
    const prepareIncomeData = () => {
        ...
        const contractsByType = payoutContracts.reduce((acc: Record<string, any[]>, contract: any) => {
        ...
        <div className="space-y-6">
            ...
            <Card>
                ...
                <CardContent className="p-0">
                            ...
                            <tbody>
                                {incomeData.map((row, index) => (
                                    <tr key={index} className="border-b hover:bg-gray-50">
                                        ...
                                        <td className="p-3 text-right text-red-600">{formatCurrency(row.solidarityTax)}</td>
                                        <td className="p-3 text-right">{formatCurrency(row.afterTax)}</td>
                                        <td className="p-3 text-right text-red-600">{formatCurrency(row.healthInsurance)}</td>
                                        <td className="p-3 text-right text-green-600 font-medium">{formatCurrency(row.afterHealthInsurance)}</td>
                                        <td className="p-3 text-right text-green-600 font-bold">{formatCurrency(row.purchasingPower)}</td>
                                    </tr>
                                ))}
                                ...
                                <tr className="border-t-2 border-gray-800 bg-gray-100 font-bold">
                                    ...
                                    <td className="p-3 text-right text-red-600">{formatCurrency(totals.incomeTax)}</td>
                                    <td className="p-3 text-right text-red-600">{formatCurrency(totals.churchTax)}</td>
                                    <td className="p-3 text-right text-red-600">{formatCurrency(totals.solidarityTax)}</td>
                                    <td className="p-3 text-right">{formatCurrency(totals.afterTax)}</td>
                                    <td className="p-3 text-right text-red-600">{formatCurrency(totals.healthInsurance)}</td>
                                    <td className="p-3 text-right text-green-600">{formatCurrency(totals.afterHealthInsurance)}</td>
                                    <td className="p-3 text-right text-green-600 text-lg">{formatCurrency(totals.purchasingPower)}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div className="p-4 text-xs text-gray-500 border-t">
                        * Alle Beträge sind monatliche Werte. Die Kaufkraft berücksichtigt die Inflation über 2% p.a.
                    </div>
                </CardContent>
            </Card>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="text-center">
                            ...
                            <div className={`text-xl font-bold ${pensionGap > 0 ? 'text-red-600' : 'text-green-600'}`}>
                        ...
                        <>
                            ...
                            <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
                                ...
                                <div className="text-xs text-gray-600 mt-3">
                                    Dies entspricht einem zusätzlichen Kapitalbedarf pro Jahr von: <strong>{formatCurrency(inflationAdjustedCapital / 20)}</strong>
                                </div>
                            </div>
                        </>
                    )}
            ...
            {pensionGap > 0 && (
                <Card className="border-orange-200 bg-orange-50">
                    <CardHeader>
                        <CardTitle className="text-orange-800">Empfehlungen zur Schließung der Versorgungslücke</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="space-y-3 text-sm">
                            <div className="flex items-start gap-2">
                                ...
                                <div>
                                    <strong>Erhöhung der privaten Altersvorsorge:</strong> Zusätzliche monatliche Einzahlungen in private Rentenversicherungen oder Fondssparpläne.
                                </div>
                            </div>
                            <div className="flex items-start gap-2">
                                <div className="w-2 h-2 bg-orange-500 rounded-full mt-2 flex-shrink-0"></div>
                                <div>
                                    <strong>Betriebliche Altersvorsorge:</strong> Maximierung der betrieblichen Vorsorge durch Entgeltumwandlung oder Arbeitgeberzuschüsse.
                                </div>
                            </div>
                            <div className="flex items-start gap-2">
                                <div className="w-2 h-2 bg-orange-500 rounded-full mt-2 flex-shrink-0"></div>
                                <div>
                                    <strong>Riester-Rente:</strong> Nutzung staatlicher Förderungen für zusätzliche Rentenerträge.
                                </div>
                            </div>
                            <div className="flex items-start gap-2">
                                <div className="w-2 h-2 bg-orange-500 rounded-full mt-2 flex-shrink-0"></div>
                                <div>
                                    <strong>Immobilieninvestments:</strong> Aufbau von Mieteinnahmen zur Ergänzung der Altersrente.
                                </div>
                            </div>
                        </div>
                    </CardContent>
